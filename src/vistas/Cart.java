...
public class Cart extends JFrame {
	...
	}, new String[] { "Codigo", "Imagen", "Producto", "Precio", "Cantidad", "Total", "Eliminar" }) {
	...
	private void initialize() {
		...
		ImageIcon carrito = new ImageIcon(new ImageIcon(Cart.class.getResource("/img/cart-logo.png")).getImage()
		...
		btnLimpiar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (modelo.getRowCount() > 0) {
					...
					JOptionPane.showMessageDialog(null, "El carrito ya está vacío.", "Error", JOptionPane.ERROR_MESSAGE);			
			}
		});
		...
		tablaCarrito.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent ev) {
				...
				if (row < tablaCarrito.getRowCount() && row >= 0 && column < tablaCarrito.getColumnCount()
						&& column >= 0) {
					...
					if (value instanceof JButton) {
						...
						if ((int) tablaCarrito.getValueAt(row, 4) == 1) {
							System.out.println("Eliminando producto con codigo " + actualizar.get(row).getCodigo()
									+ " del carrito.");
							...
							tablaCarrito.setModel(modelo);
							System.out.println("Producto " + ((Vector) item).get(0).toString()
	...
	public void ProcesarPago() {
		if (modelo.getRowCount() > 0)
			...
			JOptionPane.showMessageDialog(this, "Ingrese productos al carrito para realizar un pago.", "Error",
	...
	public void AgregarFila(String codigo, String nombre, float precio, String categoria, String url) {
		...
		ImageIcon imagenProducto = new ImageIcon(
				new ImageIcon(Cart.class.getResource(url)).getImage().getScaledInstance(50, 50, Image.SCALE_DEFAULT));
	...
	public void cargarProductos(ArrayList<Producto> productos) {
		for (Producto producto : productos) {
			AgregarFila(producto.getCodigo(), producto.getNombre(), producto.getPrecio(), producto.getCategoria(),
					producto.getImagen());
		}
	}
